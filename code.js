...
function copyToClipboard(text) {
  	window.prompt("Prekopiraj ID: Ctrl+C(Windows) ali cmd+C(Mac)", text);
}
...
function passIdToNextPage() {
	...
	window.location.href = "main.html?id="+$("#preberiEHRid").val();
}

function preberiPodatkeOsebe() {
	sessionId = getSessionId();
	...
	$.ajax({
    	...
    	success: function (data) {
			...
			$("#preberiSporocilo").html("<span class='obvestilo label label-success fade-in'>Bolnik '" + party.firstNames + " " + party.lastNames + "', ki se je rodil '" + party.dateOfBirth + "'.</span>");
			console.log("Bolnik '" + party.firstNames + " " + party.lastNames + "', ki se je rodil '" + party.dateOfBirth + "'.");
			$("#user_name").text(party.firstNames + " " + party.lastNames);
			$("#user_birtdate").text(party.dateOfBirth.split("T")[0]);
		},
		error: function(err) {
			$("#preberiSporocilo").html("<span class='obvestilo label label-danger fade-in'>Napaka '" + JSON.parse(err.responseText).userMessage + "'!");
			console.log(JSON.parse(err.responseText).userMessage);
...
function toggleDodajPodatke() {
	var isVisible = $("#dodajPodatke").is(':visible');
	    	...
	    	success: function (data) {
				...
				if (tip == "telesna temperatura") {
					...
					$.ajax({
					    ...
					    success: function (res) {
					    	if (res.length > 0) {
						    	var results = "<table class='table table-striped table-hover'><tr><th>Datum in ura</th><th class='text-right'>Pritisk (Sis/Dia)</th></tr>";
						        for (var i in res) {
						            results += "<tr><td>" + res[i].time + "</td><td class='text-right'>" + res[i].systolic + " " 	+ res[i].unit + " / " + res[i].diastolic + " " 	+ res[i].unit + "</td>";
						        }
						        results += "</table>";
						        $("#rezultatMeritveVitalnihZnakov").append(results);
					    	} else {
					...
					$.ajax({
					    ...
					    success: function (res) {
					    	if (res.length > 0) {
						    	var results = "<table class='table table-striped table-hover'><tr><th>Datum in ura</th><th class='text-right'>Kisik v krvi</th></tr>";
						        for (var i in res) {
